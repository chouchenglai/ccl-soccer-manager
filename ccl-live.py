...
# --- 1. 處理頁面跳轉參數 ---
# 定義您的標籤順序（請務必對照您下方 st.tabs 的實際順序）
...
if "target" in q_params:
    ...
    if t_key in target_map:
        st.session_state.active_tab = target_map[t_key]

# --- 2. 在頁面中使用連動的 Tabs ---
# 注意：Streamlit 的 st.tabs 本身不直接支援動態切換 index，
# 但我們可以用一個小技巧，讓內容根據參數顯示。
# 這裡建議您原本的 st.tabs 維持不變，但可以先測試連結是否能順利跳轉到該頁面。
